...
int MCP2515Class::begin(long baudRate)
{
  ...
  const struct {
    ...
    // 20MHz oscillator, derived (not in upstream library):
    // TQ = 2*(BRP+1)/Fosc, BRP=1 -> TQ=200ns, bit time=20TQ=4us=250kbps.
    // Segments: Sync(1) + PRSEG(6) + PHSEG1(7) + PHSEG2(6) = 20 TQ,
    // sample point = (1+6+7)/20 = 70% (Microchip recommends 60-70%).
    // CNF1=0x01 (SJW=1TQ, BRP=1), CNF2=0xB5 (BTLMODE=1,SAM=0,PHSEG1=7,PRSEG=6),
    // CNF3=0x05 (PHSEG2=6). Verify with a scope/logic analyzer or a second
    // known-good CAN node before trusting this on a flight-critical bus.
    { (long)20E6,  (long)250E3, { 0x01, 0xb5, 0x05 } },
  };
